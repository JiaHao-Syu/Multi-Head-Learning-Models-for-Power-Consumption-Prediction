...
class Linear_Net(torch.nn.Module):
    def __init__(self, n_time, n_output):
        ...
        self.FL3o1 = torch.nn.Linear(16,  7)
        self.FL3o2 = torch.nn.Linear(16, 13)
        self.FL3o3 = torch.nn.Linear(16,  3)
        
        self.SL1 = torch.nn.Linear(23, 16)
    ...
    def forward(self, x):
        ...
        F1 = self.FL3o1(self.LRelu(self.FL2o1(torch.sigmoid(self.FL1o1(x1)))))
        F2 = self.FL3o2(self.LRelu(self.FL2o2(torch.sigmoid(self.FL1o2(x2)))))
        F3 = self.FL3o3(self.LRelu(self.FL2o3(torch.sigmoid(self.FL1o3(x3)))))
...
def LW(l0, l1, l2, l3):
    B, W = np.std(l0), [1,1,1,1]
    if np.std(l1)!=0:  W[1] = B/np.std(l1)
    if np.std(l2)!=0:  W[2] = B/np.std(l2)
    if np.std(l3)!=0:  W[3] = B/np.std(l3)
    for i in range(4): W[i] = min(max(W[i], 0.1), 10)
    return W
...
for im, model_name in enumerate(MN):
    ...
    for epoc in range(EPOC):
        ...
        for b in range(BN):
            t, f = BI[b], BI[b+1]
            pred, f1, f2, f3 = net(trainx[t:f])
            optimizer.zero_grad()
            loss = loss_fn(torch.reshape(pred ,(-1,)), trainy[t:f])
            LP0.append( float(loss) )
            loss.backward(retain_graph=True)
            loss = loss_fn(f1, trainf[t:f,:7])*wei[1]
            LF1.append( float(loss) )
            loss.backward(retain_graph=True)
            loss = loss_fn(f2, trainf[t:f,7:20])*wei[2]
            LF2.append( float(loss) )
            loss.backward(retain_graph=True)
            loss = loss_fn(f3, trainf[t:f,20:])*wei[3]
            LF3.append( float(loss) )
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
        wei = LW(LP0, LF1, LF2, LF3)
        LP0, LF1, LF2, LF3 = sum(LP0), sum(LF1), sum(LF2), sum(LF3)
        pred, f1, f2, f3 = net(trainx)
        loss = float(loss_fn(torch.reshape(pred,(-1,)), trainy))
        if epoc%10==9: 
            SUM = LP0+LF1+LF2+LF3
            print(epoc+1, '/', EPOC, ' ', round(LP0/SUM,3), round(LF1/SUM,3), round(LF2/SUM,3), round(LF3/SUM,3))
        if loss<BL:
            BL = loss
    ...
    for ir, route in enumerate(['B', 'C']):
        ...
        net.SL1.weight = Net.SL1.weight
        net.SL2.weight = Net.SL2.weight
        net.SL3.weight = Net.SL3.weight
        net.SL1.bias   = Net.SL1.bias
        net.SL2.bias   = Net.SL2.bias
        net.SL3.bias   = Net.SL3.bias
        net.to(device)
...
np.save('3-2StageFPLC_'+str(NPERIOD)+'_'+str(EPOC)+'.npy', [TRAIN_LOSS, TEST_LOSS])
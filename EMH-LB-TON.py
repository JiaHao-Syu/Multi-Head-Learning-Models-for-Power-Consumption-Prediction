...
for im, model_name in enumerate(MN):
    ...
    for epoc in range(EPOC):
        ...
        for b in range(BN):
            ...
            loss = loss_fn(torch.reshape(pred1,(-1,)), trainy[t:f])/wei[1]
            LP1.append( float(loss) )
            loss.backward(retain_graph=True)
            loss = loss_fn(torch.reshape(pred2,(-1,)), trainy[t:f])/wei[2]
            LP2.append( float(loss) )
            loss.backward(retain_graph=True)
            loss = loss_fn(torch.reshape(pred3,(-1,)), trainy[t:f])/wei[3]
            LP3.append( float(loss) )
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
        wei = LW(LP0, LP1, LP2, LP3)
        LP0, LP1, LP2, LP3 = sum(LP0), sum(LP1), sum(LP2), sum(LP3)
        pred, pred1, pred2, pred3 = net(trainx)
        loss = float(loss_fn(torch.reshape(pred,(-1,)), trainy))
        if epoc%10==9: 
            SUM = LP0+LP1+LP2+LP3
            print(epoc+1, '/', EPOC, ' ', round(LP0/SUM,3), round(LP1/SUM,3), round(LP2/SUM,3), round(LP3/SUM,3))
        if loss<BL:
            BL = loss
...
np.save('3-2StagePPLC_'+str(NPERIOD)+'_'+str(EPOC)+'.npy', [TRAIN_LOSS, TEST_LOSS])